                                        ...
                                        tpdfcplx_impl_ *cyl_y_arr, int scaled)
{
    ...
    if (cabs(z) < DBL_EPSILON) {
        ...
        if (fabs(floor(fnu) - fnu) < DBL_EPSILON) {
            ...
            for (int i = 0; i < n; i++) {
                int tmp = n - 1 - i;
                cyl_y_arr[i] = pow(-1.0, fnu_m + tmp) * (cyr_ptr[tmp] + I_IMPL_ * cyi_ptr[tmp]);
            }
        ...
        if (fabs(floor(fnu) - fnu) < DBL_EPSILON) {
            ...
            for (int i = 0; i < n_m; i++) {
                int tmp = n_m - 1 - i;
                cyl_y_arr[i] = pow(-1.0, fnu_m + tmp) * (cyr_m_ptr[tmp] + I_IMPL_ * cyi_m_ptr[tmp]);
            }
            ...
            for (int i = 0; i < n_m; i++) {
                ...
                double tmp2 = (fnu_m + (double)tmp1) * M_PI;
                /* Eq. (5.5.4) of Ref. [2] */
                cyl_y_arr[i] = (cjr_m_ptr[tmp1] + I_IMPL_ * cji_m_ptr[tmp1]) * sin(tmp2) +

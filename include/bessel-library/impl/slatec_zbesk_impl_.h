...
/* DECK ZBESK */
/* Subroutine */
static inline int slatec_zbesk_impl_(double *zr, double *zi, double *fnu, int *kode, int *n,
                                     double *cyr, double *cyi, int *nz, int *ierr)
{
    ...
    *nz = 0;
    ...
    goto L60;
L50:
    slatec_zuoik_impl_(zr, zi, fnu, kode, &c_u_2, &nn, &cyr[1], &cyi[1], &nuf, &tol, &elim, &alim);
    if (nuf < 0) {
        goto L180;
    }
    *nz += nuf;
...
L90:
    slatec_zbunk_impl_(zr, zi, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &tol, &elim, &alim);
    if (nw < 0) {
        goto L200;
    }
    *nz += nw;

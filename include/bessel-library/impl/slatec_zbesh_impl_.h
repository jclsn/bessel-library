...
/* DECK ZBESH */
/* Subroutine */
static inline int slatec_zbesh_impl_(double *zr, double *zi, double *fnu, int *kode, int *m, int *n,
...
L70:
    ...
    /*     YN.GE.0. .OR. M=1) */
    /* ----------------------------------------------------------------------- */
    slatec_zbknu_impl_(&znr, &zni, fnu, kode, &nn, &cyr[1], &cyi[1], nz, &tol, &elim, &alim);
    goto L110;
/* ----------------------------------------------------------------------- */
...
L90:
    ...
    zni = -zni;
L100:
    slatec_zbunk_impl_(&znr, &zni, fnu, kode, &mr, &nn, &cyr[1], &cyi[1], &nw, &tol, &elim, &alim);
    if (nw < 0) {
        goto L240;
    }
    *nz += nw;

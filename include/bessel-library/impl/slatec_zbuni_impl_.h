...
/* DECK ZBUNI */
/* Subroutine */
static inline int slatec_zbuni_impl_(double *zr, double *zi, double *fnu, int *kode, int *n,
                                     ...
                                     double *fnul, double *tol, double *elim, double *alim)
{
    ...
    static double ax, ay;
    static int nl, nw;
    static double c1i, c1m, c1r, s1i, s2i, s1r, s2r, cyi[2], gnu, raz, cyr[2], sti, bry[3], rzi,
        str, rzr, dfnu;
    static double fnui;
    static int iflag;
    ...
    *nz = 0;
    ...
    /* ----------------------------------------------------------------------- */
    slatec_zuni1_impl_(zr, zi, &gnu, kode, &c_u_2, cyr, cyi, &nw, nlast, fnul, tol, elim, alim);
    goto L20;
L10:
    /* ----------------------------------------------------------------------- */
    ...
    /*     AND HPI=PI/2 */
    /* ----------------------------------------------------------------------- */
    slatec_zuni2_impl_(zr, zi, &gnu, kode, &c_u_2, cyr, cyi, &nw, nlast, fnul, tol, elim, alim);
L20:
    if (nw < 0) {
        goto L50;
    }
...
L60:
    ...
    /*     -PI/3.LE.ARG(Z).LE.PI/3 */
    /* ----------------------------------------------------------------------- */
    slatec_zuni1_impl_(zr, zi, fnu, kode, n, &yr[1], &yi[1], &nw, nlast, fnul, tol, elim, alim);
    goto L80;
L70:
    /* ----------------------------------------------------------------------- */
    /*     ASYMPTOTIC EXPANSION FOR J(FNU,Z*EXP(M*HPI)) FOR LARGE FNU */
    ...
    /*     AND HPI=PI/2 */
    /* ----------------------------------------------------------------------- */
    slatec_zuni2_impl_(zr, zi, fnu, kode, n, &yr[1], &yi[1], &nw, nlast, fnul, tol, elim, alim);
L80:
    if (nw < 0) {
        goto L50;
    }

...
/* DECK ZBINU */
/* Subroutine */
static inline int slatec_zbinu_impl_(double *zr, double *zi, double *fnu, int *kode, int *n,
...
L50:
    ...
    /* ----------------------------------------------------------------------- */
    slatec_zuoik_impl_(zr, zi, fnu, kode, &c_u_1, &nn, &cyr[1], &cyi[1], &nw, tol, elim, alim);
    if (nw < 0) {
        goto L130;
    }
    *nz += nw;
...
L100:
    ...
    slatec_zwrsk_impl_(zr, zi, fnu, kode, &nn, &cyr[1], &cyi[1], &nw, cwr, cwi, tol, elim, alim);
    if (nw < 0) {
        goto L130;
    }
    goto L120;

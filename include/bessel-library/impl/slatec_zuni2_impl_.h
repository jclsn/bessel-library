...
/* DECK ZUNI2 */
/* Subroutine */
static inline int slatec_zuni2_impl_(double *zr, double *zi, double *fnu, int *kode, int *n,
                                     ...
                                     double *tol, double *elim, double *alim)
{
    ...
    static int nuf, inu;
    static double bry[3], raz, sti, zbr, zni, cyr[2], rzi, str, znr, rzr, daii, cidi, aarg;
    static int ndai;
...
L120:
    ...
    slatec_zuoik_impl_(zr, zi, fnu, kode, &c_u_1, &nd, &yr[1], &yi[1], &nuf, tol, elim, alim);
    if (nuf < 0) {
        goto L140;
    }
    nd -= nuf;

...
/* DECK ZBKNU */
/* Subroutine */
static inline int slatec_zbknu_impl_(double *zr, double *zi, double *fnu, int *kode, int *n,
                                     ...
                                     double *alim)
{
    ...
    static int nw;
    static double p1i, p2i, s1i, s2i, p2m, p1r, p2r, s1r, s2r, cbi, cbr, cki, caz, csi, ckr, fhs,
        fks, rak, czi, dnu, csr, elm, zdi, bry[3], pti, czr, sti, zdr, cyr[2], rzi, ptr, cyi[2];
    static int inu;
...
L280:
    ascle = bry[0];
    slatec_zkscl_impl_(&zdr, &zdi, fnu, n, &yr[1], &yi[1], nz, &rzr, &rzi, &ascle, tol, elim);
    inu = *n - *nz;
    if (inu <= 0) {
        return 0;
    }
